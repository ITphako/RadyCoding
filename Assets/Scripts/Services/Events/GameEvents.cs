  ...
  public class GameEvents :  IGameEventsListener, IGameEventsExecuter
    {
        ...
        public void OnLevelLose() => LevelLose?.Invoke();
         ...
         public void OnActorSpawned(Actor who) =>   ActorSpawned?.Invoke(who);

           public void OnActorCollisionActor(Actor who, Actor target) =>  ActorCollisionActor?.Invoke(who, target);

             public void OnActorCollisionExitActor(Actor who, Actor target) => ActorCollisionExitActor?.Invoke(who, target);

             public void OnActorLeftGround(Actor actor) =>   ActorLeftGround?.Invoke(actor);

             public void OnActorDied(Actor actor) => ActorDied?.Invoke(actor);
    }